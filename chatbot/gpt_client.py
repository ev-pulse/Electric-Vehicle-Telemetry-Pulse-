...
def convert_to_sql(user_question: str, schema: str) -> str:
    ...
    - 자치구 이름으로 질문하면 해당 행정구역 코드로 변환해서 조회  예: 마포구 → 140, 강남구 → 230
    - 매핑표:종로구=010, 중구=020, 용산구=030, 성동구=040, 광진구=050,동대문구=060, 중랑구=070, 성북구=080, 강북구=090, 도봉구=100,
            노원구=110, 은평구=120, 서대문구=130, 마포구=140, 양천구=150,강서구=160, 구로구=170, 금천구=180, 영등포구=190, 동작구=200,
            관악구=210, 서초구=220, 강남구=230, 송파구=240, 강동구=250
    - 지역 계층 규칙:
    - 쿼리 지역이 광역자치단체(시/도)이면, 해당 지역 산하 모든 기초자치단체(구/시/군) 데이터를 합산하라.
    - 쿼리 지역이 "전국"이면 17개 광역자치단체 전체를 합산하라.
    - 데이터가 없는 하위 지역은 누락 사실을 명시하라.
